...
def sort_box(box):
    ...
    timer.toc()
    print("\n----------------------------------------------")
    print(('sort_box took {:.3f}s').format(timer.total_time))
    return box
...
def dumpRotateImage(img, degree, pt1, pt2, pt3, pt4):
    ...
    imgOut = imgRotation[max(1, int(pt1[1])): min(
        ydim - 1, int(pt3[1])), max(1, int(pt1[0])): min(xdim - 1, int(pt3[0]))]
...
def charRec2(img, text_recs, adjust=False):
    """
    加载OCR模型，进行字符识别
    """
    results = {}
...
image = np.array(Image.open("./test_images/1.jpg").convert('RGB'))
model(image)
